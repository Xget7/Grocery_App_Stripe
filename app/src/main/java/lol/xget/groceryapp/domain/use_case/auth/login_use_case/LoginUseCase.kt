package lol.xget.groceryapp.domain.use_case.auth.login_use_case
...
class LoginUseCase @Inject constructor(
    ...
    operator fun invoke(email: String, password : String): Flow<Resource<LoginState>> = callbackFlow {
        try {
            trySend(Resource.Loading())
                repository.signInWithEmailPassword(email,password).addOnSuccessListener {
                    try {
                        trySend(Resource.Success(LoginState(successLogin = true, email = email, password = password)))
                    }catch (e: Exception){
                        cancel()
                    }
