...
class GetShopProducts @Inject constructor(
    ...
    operator fun invoke(userId : String): Flow<Resource<SellerHomeState>> = callbackFlow {
            val fbProductList = mutableListOf<lol.xget.groceryapp.seller.mainSeller.domain.ProductModel>()
            try {
                try {
                    ...
                    repo.getProductsData(userId).addValueEventListener(object : ValueEventListener {
                        ...
                        override fun onDataChange(snapshot: DataSnapshot) {
                            for (ds in snapshot.children) {
                                val productModel = ds.getValue(lol.xget.groceryapp.seller.mainSeller.domain.ProductModel::class.java)
