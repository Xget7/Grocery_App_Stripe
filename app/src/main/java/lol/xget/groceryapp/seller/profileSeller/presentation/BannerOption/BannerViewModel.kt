...
class BannerViewModel @Inject constructor(
    ...
    val state: MutableState<BannerState> = mutableStateOf(BannerState())
    ...
    @ExperimentalCoroutinesApi
    fun updateProfile(user: User, adBanner: Uri?, shopBanner: Uri?) {
        repo.updateShopBanners.invoke(user, adBanner, shopBanner).onEach { result ->
            when (result) {
                is Resource.Loading -> {
                    state.value = state.value.copy(loading = true)
                }
